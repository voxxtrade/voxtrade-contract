...
#[contractimpl]
impl MockEscrow {
    pub fn lock_funds(_env: Env, _buyer: Address, _seller: Address, _token: Address, _amount: i128, _hash_lock: BytesN<32>, _timeout: u32) -> u64 {
        1
    }
}
...
fn test_rolling_limit() {
    ...
    // Spend 60 -> Success
    treasury.execute_x402_lock(&agent, &token, &escrow_id, &seller, &60_i128, &hash_lock, &timeout);

    // Spend 50 -> Fail (60 + 50 = 110 > 100)
    let res = treasury.try_execute_x402_lock(&agent, &token, &escrow_id, &seller, &50_i128, &hash_lock, &timeout);
    ...
    treasury.execute_x402_lock(&agent, &token, &escrow_id, &seller, &50_i128, &hash_lock, &timeout);
}
