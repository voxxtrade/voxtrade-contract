...
- **Guaranteed Refunds on Timeout** &mdash; If a seller fails to deliver the service preimage before the \	imeout_ledger\, the buyer is guaranteed a full refund (\TimeoutReached\ / \TimeoutNotReached\).
- **Zero Double-Spending** &mdash; An escrow can only be claimed or refunded exactly once. Once \esolved\, all subsequent interactions are blocked (\AlreadyResolved\).
